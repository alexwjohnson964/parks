...
app.locals.baseUrl = '/parks';
const LokiStore = store(session);
const PgPersistence = require("./lib/pg-persistence");
app.set("views", "./views");
app.set("view engine", "pug");
//app.use(express.static("./public"));
app.use('/parks', express.static(path.join(__dirname, 'public')));
...
const requiresAuthentication = (req, res, next) => {
  if (!req.session.signedIn) {
    res.redirect(302, `${app.locals.baseUrl}/signin`);
...
router.get("/lists/all-lists/",
  requiresAuthentication,
  catchError(async (req, res) => {
    res.redirect(`${app.locals.baseUrl}/lists/all-lists/1`);
  })
);
router.get("/lists/all-lists/:pageNumber",
  requiresAuthentication,
  catchError(async (req, res) => {
...
router.get("/lists/:listId/:pageNumber",
  requiresAuthentication,
  catchError(async (req, res) => {
    ...
    if (pageNumber > list.parks.length / 5 + 1 || !(Number.isInteger(pageNumber)) || pageNumber < 1) {
      req.flash("error", `Invalid page number`);
      pageNumber = 1;
      return res.redirect(`${app.locals.baseUrl}/lists/${listId}/1`);
    } 
    
    res.render("list", {sortBy: req.session.sortBy, list, pageNumber});
  })
);
router.get("/park/create",
  requiresAuthentication,
  catchError(async (req, res) => {
...
router.post("/park/:parkId/edit-rating/:rating",
  requiresAuthentication,
  catchError(async (req, res) => {
    ...
    if (updated) {
        ...
        res.redirect(`${app.locals.baseUrl}/park/${parkId}`);
    } else {
      req.flash("error", "Failed to update.");
      res.redirect(`${app.locals.baseUrl}/parks/${parkId}`)
    }
 })
);
router.post("/park/:parkId/edit-notes",
  requiresAuthentication,
  body('parkNotes'),
  [
...
router.post("/park/:parkId/delete",
  requiresAuthentication,
  catchError(async (req, res) => {
    let parkId = req.params.parkId;
...
router.post("/lists/edit/:listId/:parkId",
  requiresAuthentication,
  catchError(async (req, res) => {
...
router.post("/lists/:listId/edit",
  ...
  res.redirect(`${app.locals.baseUrl}/lists/${listId}/1`);
 })
);
router.post("/lists/:listId/sort",
  requiresAuthentication,
  body('sortBy'),
  catchError(async (req, res) => {
    ...
    if (list) {
        req.flash("success", `The list has been updated.`);
        res.redirect(`${app.locals.baseUrl}/lists/${req.params.listId}/1`);
    } else {
...
router.post("/lists/:listId/delete",
  requiresAuthentication,
  catchError(async (req, res) => {
...
router.post("/signout", (req, res) => {
  req.session.destroy((err) => {
    ...
    res.redirect(`${app.locals.baseUrl}/signin`);
  });
});
...
router.post("/signin", async (req, res)  => {
  ...
  if (!validCredentials) {
    ...
    let destination = req.session.destination || `${app.locals.baseUrl}/`;
